...
interface CartRow {
  ...
  quantity: number;
  productId: number;
  product: { id: number; name: string; priceUsd: string; imageUrl: string; purity: string };
}
...
export default function CartPage() {
  ...
  const total = items.reduce(
    (s, i) => s + parseFloat(i.product.priceUsd) * i.quantity,
    0
  );
    ...
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 text-stone-500">
          ...
          <Link href="/">
            <span className="btn-gold cursor-pointer">Browse Products</span>
          </Link>
        </div>
      ) : (
        <>
            ...
            {items.map((item) => (
              <div key={item.id} className="card p-4 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-stone-500 text-sm">{item.product.purity}</p>
                </div>
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => updateQty(item.id, item.quantity - 1)}
                    className="w-7 h-7 rounded-full bg-stone-800 hover:bg-stone-700 flex items-center justify-center text-stone-400"
                  >
                    <Minus size={12} />
                  </button>
                ...
                <div className="text-gold-400 font-semibold text-sm w-20 text-right">
                  ${(parseFloat(item.product.priceUsd) * item.quantity).toLocaleString()}
                </div>
                <button
